...
public class LazyViewPager extends ViewGroup {
    ...
    void populate() {
        ...
        if (DEBUG) {
            ...
            for (int i=0; i<mItems.size(); i++) {
                Log.i(UHFMainActivity.TAG, "#" + i + ": page " + mItems.get(i).position);
            }
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP) {
            // Release the drag.
            if (DEBUG) Log.v(UHFMainActivity.TAG, "Intercept done!");
        ...
        if (action != MotionEvent.ACTION_DOWN) {
            if (mIsBeingDragged) {
                if (DEBUG) Log.v(UHFMainActivity.TAG, "Intercept returning true!");
                return true;
            }
            if (mIsUnableToDrag) {
                if (DEBUG) Log.v(UHFMainActivity.TAG, "Intercept returning false!");
                return false;
            }
        }
